...
> & {
  transform?: UseTableOptions<ResponseData, ApiData, NaiveUI.TableColumn<ApiData>, Pagination>['transform'];
  /**
   * get column visible
   *
   * @param column
...
type LooseNaiveTableOptions = {
  api?: (...args: any[]) => Promise<any>;
  transform?: (response: any) => any;
  columns?: () => NaiveUI.TableColumn<any>[];
  getColumnVisible?: (column: NaiveUI.TableColumn<any>) => boolean;
...
export function useNaiveTable<ResponseData = any, ApiData = any>(options: LooseNaiveTableOptions) {
  ...
  const result = useTable<ResponseData, ApiData, NaiveUI.TableColumn<ApiData>, false>({
    ...
    getColumns
  } as UseTableOptions<ResponseData, ApiData, NaiveUI.TableColumn<ApiData>, false>);
...
export function useNaivePaginatedTable<ResponseData = any, ApiData = any>(options: LooseNaiveTableOptions) {
  const scope = effectScope();
  const appStore = useAppStore();
  ...
  // 默认 transform：兼容 RuoYi { rows, total } 格式和标准 { data, total } 格式
  const defaultTransform = (response: any) => ({
    data: response?.data?.rows ?? response?.data?.data ?? response?.rows ?? [],
    total: response?.data?.total ?? response?.total ?? 0,
    pageNum: 1
  });

  const { transform = defaultTransform as any, ...restOptions } = options as any;
  ...
  } as UseTableOptions<ResponseData, ApiData, NaiveUI.TableColumn<ApiData>, true>);
...
export function useTableOperate<TableData = any>(getData: () => Promise<void>): ReturnType<
  typeof createTableOperate<TableData>
>;
export function useTableOperate<TableData = any>(
  data: Ref<TableData[]>,
  idKey: keyof TableData,
  getData: () => Promise<void>
): ReturnType<typeof createTableOperate<TableData>>;
export function useTableOperate<TableData = any>(
  dataOrGetData: Ref<TableData[]> | (() => Promise<void>),
  idKey?: keyof TableData,
  getDataArg?: () => Promise<void>
) {
  const data = typeof dataOrGetData === 'function' ? ref<TableData[]>([]) : dataOrGetData;
  const getData = typeof dataOrGetData === 'function' ? dataOrGetData : getDataArg!;

  return createTableOperate(data as Ref<TableData[]>, idKey, getData);
}

function createTableOperate<TableData = any>(
  data: Ref<TableData[]>,
  idKey: keyof TableData | undefined,
  getData: () => Promise<void>
) {
  ...
  function handleEdit(id: TableData[keyof TableData] | TableData) {
    operateType.value = 'edit';
    const findItem = idKey ? data.value.find(item => item[idKey] === id) || null : (id as TableData);
...
export function defaultTransform<ApiData>(
  ...
  const records = (payload.rows as ApiData[] | undefined) ?? (Array.isArray(data) ? (data as ApiData[]) : []);
