...
const _realRequest = createFlatRequest(
  {
    baseURL,
    'axios-retry': {
      retries: 0
  ...
  },
  {
    ...
    async onBackendFail(response, instance) {
      ...
      function logoutAndCleanup() {
        ...
        _realRequest.state.errMsgStack = _realRequest.state.errMsgStack.filter(msg => msg !== response.data.msg);
      }
      ...
      if (modalLogoutCodes.includes(responseCode) && isLogin) {
        const isExist = _realRequest.state.errMsgStack?.includes(response.data.msg);
        if (isExist) {
          return null;
        }
        ...
        _realRequest.state.errMsgStack = [...(_realRequest.state.errMsgStack || []), response.data.msg];
      ...
      if (expiredTokenCodes.includes(responseCode)) {
        const success = await handleExpiredRequest(_realRequest.state);
        if (success) {
          const Authorization = getAuthorization();
    ...
    onError(error) {
      ...
      showErrorMsg(_realRequest.state, message);
    }
  }
);
  ...
  if (isMockMode()) {
    try {
      ...
      if (responseType === 'blob' || responseType === 'arraybuffer') {
        const blob =
          data instanceof Blob ? data : new Blob([typeof data === 'string' ? data : JSON.stringify(data ?? '')]);
        return { data: blob as any, error: null, response: undefined };
      }
      return { data, error: null, response: undefined };
    } catch (error) {
      return { data: null, error, response: undefined };
    }
  }
  return _realRequest(config);
}) as typeof _realRequest;

request.cancelAllRequest = _realRequest.cancelAllRequest;
request.state = _realRequest.state;
