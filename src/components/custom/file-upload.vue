...
function beforeUpload(options: { file: UploadFileInfo; fileList: UploadFileInfo[] }) {
  ...
  // 校检文件类型（扩展名忽略大小写，与 isImage 等工具函数一致）
  if (accept.value) {
    const rawName = (file.name || file.file?.name || '').trim();
    const dotIndex = rawName.lastIndexOf('.');
    if (dotIndex <= 0 || dotIndex === rawName.length - 1) {
      window.$message?.error('文件缺少有效扩展名，请使用带后缀的文件（如 .pdf）');
      return false;
    }
    const fileExt = rawName.slice(dotIndex).toLowerCase();
    const allowedExts = accept.value.split(',').map(ext => ext.trim().toLowerCase());
